...
class HandTracker extends React.Component {
		...
		camera = new cam.Camera(webCamRef.current.video, {
			onFrame: async () => {
				if (this.state.initialState) {
					this.setState({
						initialImage: webCamRef.current.video,
						initialState: true
					});
				}
...
function captureInitialImage() {
	...
	camera.start();//change this to state
	//get a callback to stop camera after initial image
}
	...
	<div className="container-hand-tracker">
		...
		<div className="container-data">
			...
			<form className="utility-container">
				...
				<button className="button-csv" onClick={captureInitialImage}>Capture Initial Image</button>
			</form>
		</div>
	</div>
)
